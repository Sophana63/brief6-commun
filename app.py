            ...
            [
                html.Div(
                    [
                        ...
                        dcc.Dropdown(
                            id="pays_menu",
                            options=["Greece", "Great Britain", "Australia", "Japan", "Spain", "Canada", "Brazil", "France"],
                            multi=True,
                            value=list(["Great Britain", "France"]),
                            className="dcc_control",
                        ),
                    ],
...
def filter_dataframe(df, pays, sexe, year_slider):
    if sexe == "All":
        dff = df[
            ...
            & (df["date"] >= year_slider[0])
            & (df["date"] <= year_slider[1])
        ]
...
def filter_dataframe_without_date(df, pays, sexe):
    if sexe == "All":
        dff = df[
            (df["country_name"].isin(pays))
        ]
...
def update_figure(selected_year, pays, sexe):
    ...
    dff_without_date = filter_dataframe_without_date(df, pays, sexe)

    medal_count = dff_without_date.groupby(by="date").count().medal_type
    athlete_count = dff_without_date.groupby(by="date").count().athlete_url
...
def update_img_best(year_slider, pays, sexe):
    ...
    if not dff.medal_type.notnull().values.any():
        ...
        if url_best.split("/")[-1].split("-")[1] == 'dimas':
            img_str = "greek_man.jpg"

        elif url_best.split("/")[-1].split("-")[1] == 'korakaki':
            img_str = "greek_woman.jpg"
